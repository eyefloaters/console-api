...
class KafkaClustersResourceIT {
    ...
    @ParameterizedTest
    @CsvSource({
        "'name' , 'test-kafka1,test-kafka2,test-kafka3'",
        "'-name', 'test-kafka3,test-kafka2,test-kafka1'"
    })
    void testListClustersSortedWithPagination(String sortParam, String expectedNameList) {
        client.resources(Kafka.class)
            .resource(utils.buildKafkaResource("test-kafka3",
                    UUID.randomUUID().toString(),
                    randomBootstrapServers))
            .create();

        String[] expectedNames = expectedNameList.split(",");
        ...
        for (int i = 0, m = expectedNames.length; i < m; i++) {
            ...
            whenRequesting(req -> req
                ...
                .assertThat()
                .statusCode(is(Status.OK.getStatusCode()))
                .body("meta.page.total", equalTo(3))
